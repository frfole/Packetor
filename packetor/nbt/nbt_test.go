package nbt

import (
	"bytes"
	"compress/gzip"
	"io"
	"reflect"
	"testing"
)

func TestReadNbt(t *testing.T) {
	type args struct {
		reader io.Reader
	}
	tests := []struct {
		name      string
		args      args
		wantValue Compound
		wantErr   bool
		skip      bool
	}{
		{
			name: "Bananrama",
			args: args{reader: wrapReader([]byte{
				0x08, 0x00, 0x04, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x09,
				0x42, 0x61, 0x6e, 0x61, 0x6e, 0x72, 0x61, 0x6d, 0x61,
			})},
			wantValue: Compound(map[string]Tag{"name": String("Bananrama")}),
			wantErr:   false,
			skip:      false,
		},
		{
			name: "hello world Bananrama",
			args: args{reader: wrapReader([]byte{
				0x0a, 0x00, 0x0b, 0x68, 0x65, 0x6c, 0x6c, 0x6f,
				0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x08, 0x00,
				0x04, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x09, 0x42,
				0x61, 0x6e, 0x61, 0x6e, 0x72, 0x61, 0x6d, 0x61,
				0x00,
			})},
			wantValue: Compound(map[string]Tag{
				"hello world": Compound(map[string]Tag{
					"name": String("Bananrama"),
				}),
			}),
			wantErr: false,
			skip:    false,
		},
		{
			name: "big test",
			args: args{reader: wrapGZipReader([]byte{
				,
			})},
			wantValue: Compound(map[string]Tag{
				"hello world": Compound(map[string]Tag{
					"name": String("Bananrama"),
				}),
			}),
			wantErr: false,
			skip:    true, // I am to lazy to actually compare it to correct result
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotValue, err := ReadNbt(tt.args.reader)
			if (err != nil) != tt.wantErr {
				t.Errorf("ReadNbt() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotValue, tt.wantValue) {
				if !tt.skip {
					t.Errorf("ReadNbt() gotValue = %v, want %v", gotValue, tt.wantValue)
				}
			}
		})
	}
}

func wrapReader(data []byte) io.Reader {
	return bytes.NewReader(data)
}

func wrapGZipReader(data []byte) io.Reader {
	reader, err := gzip.NewReader(bytes.NewReader(data))
	if err != nil {
		panic(err)
	}
	return reader
}
